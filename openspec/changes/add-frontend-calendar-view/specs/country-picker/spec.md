...
The country picker SHALL display recently used countries for quick selection.

#### Scenario: Recent countries display
...
- **AND** up to 5-10 recent countries SHALL be shown as chips
...
The country picker SHALL display a scrollable list of all countries.

#### Scenario: Country list display
- **WHEN** the country picker is open
- **THEN** all 249 countries SHALL be available in the list
- **AND** countries SHALL be sorted alphabetically by name
- **AND** each country SHALL display: checkbox, flag/emoji, name
...
- **THEN** the country's assigned color SHALL be visible
